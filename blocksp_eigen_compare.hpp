...
namespace eigen_comparison
{
	...
	template<typename T>
	Eigen::Vector<T, Eigen::Dynamic> mtxVector_to_EigenVector(const std::string& vec_loc,
		...
		bool diag = false, bool robust = false, bool dr = true)
	{
		...
		A_bsp.shrink_to_fit(); //this function shrinks a BlockSp vectors' storage.
		auto ACompr_bsp{ A_bsp.compress() }; //this creates a compressed BlockSp matrix.
		...
		if (noPre)
		{
			...
			{
				start = std::chrono::steady_clock::now();
				auto x_bsp{ BlockSp::pGMRES<T, B, D>(ACompr_bsp, b_bsp, tol, [](const auto& x) {}, 2000, -1, -1, true) };
			...
			{
				start = std::chrono::steady_clock::now();
				auto x_bsp{ BlockSp::pGMRES<T, B, D>(ACompr_bsp, b_bsp, tol, [](const auto& x) {}, 2000, 30, 10, true) };
		...
		if (diag)
		{
			//pGMRES with digaonal preconditioner, no restart.
			{
				...
				auto x_bsp{ BlockSp::pGMRES<T, B, D>(ACompr_bsp, b_bsp, tol, diag, 5000, -1, -1, true) };
			...
			if (dr)
			{
				start = std::chrono::steady_clock::now();
				BlockSp::preConditioners::Diag<BlockSp::dense::LUFactor<T, B* D>, T, B, D> diag(A_bsp);
				auto x_bsp{ BlockSp::pGMRES<T, B, D>(ACompr_bsp, b_bsp, tol, diag, 5000, 30, 10, true) };
		...
		{
			start = std::chrono::steady_clock::now();
			BlockSp::SOR_pGMRES<T, B, D> sor_pgmres(A_bsp);
			sor_pgmres.change_tol(tol); sor_pgmres.change_printIters(true);
			auto x_bsp{ sor_pgmres.solve(ACompr_bsp, b_bsp) };
		...
		{
			start = std::chrono::steady_clock::now();
			BlockSp::SOR_pGMRES<T, B, D> sor_pgmres_dr(A_bsp, 1.6, 3, 0, tol, 5000, 30, 10, true);
			auto x_bsp{ sor_pgmres_dr.solve(ACompr_bsp, b_bsp) };
		...
		{
			start = std::chrono::steady_clock::now();
			BlockSp::ILU0_pGMRES<T, B, D> ilu0_pgmres(A_bsp, tol);
			ilu0_pgmres.change_printIters(true);
			auto x_bsp{ ilu0_pgmres.solve(ACompr_bsp, b_bsp) };
		...
		{
			start = std::chrono::steady_clock::now();
			BlockSp::ILU0_pGMRES<T, B, D> ilu0_pgmres(A_bsp, tol, 5000, 30, 10, true);
			auto x_bsp{ ilu0_pgmres.solve(ACompr_bsp, b_bsp) };
		...
		if constexpr (!BlockSp::is_complex_v<T>)
			if (SPD)
			{
				...
				{
					start = std::chrono::steady_clock::now();
					auto x_bsp{ BlockSp::pCG<T, B, D>(ACompr_bsp, b_bsp, tol, [](const auto&) {}, 2000, true) };
				...
				if (diag)
				{
					...
					auto x_bsp{ BlockSp::pCG<T, B, D>(ACompr_bsp, b_bsp, tol, diag, 5000, true) };
				...
				{
					start = std::chrono::steady_clock::now();
					BlockSp::SOR_pCG<T, B, D> sor_pcg(A_bsp);
					sor_pcg.change_tol(tol); sor_pcg.change_printIters(true);
					auto x_bsp{ sor_pcg.solve(ACompr_bsp, b_bsp) };
				...
				{
					start = std::chrono::steady_clock::now();
					BlockSp::IChol0_pCG<T, B, D> ichol0_pcg(A_bsp, tol, 5000, true);
					auto x_bsp{ ichol0_pcg.solve(ACompr_bsp, b_bsp) };
	...
	void LDG_2D_vecLap_test()
	{
		...
		auto x{ mtxVector_to_EigenVector<double>(mtx_x, 1) };

		std::string mtx_b = "test_mats/LDG_2D_vecLap_b.mtx";
		auto b{ mtxVector_to_EigenVector<double>(mtx_b, 1) };

		//Compare Eigen and BlockSp, random normal solution
		eigen_compare<double, B, D>(A_trip, &x, &b, true, true, true);
		...
		unsigned int start_line, bool symmetric, bool base_zero, bool printMatSize)
	{
		...
		BlockSp::util::GotoLine(f, start_line);
		...
		unsigned int start_line, bool printVecSize)
	{
		...
		//3+) values of vector
		BlockSp::util::GotoLine(f, start_line); //user must ensure starting line is vector size line 
		std::string size; std::getline(f, size);
		std::stringstream ss(size);
		int M, N; ss >> M; ss >> N;
		assert(N == 1);
