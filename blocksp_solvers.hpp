...
//		Note: these solvers do not use the fast compressed BlockSp_Compr form. 
//		Also the precondioner is constructed each call. The solvers of (2) may be more applicable.
//			ez::sor_pGMRES					--  SOR pGMRES with pre-packaged parameters for easy use. Not restarted.
//			ez::sor_pGMRES_robust   --  Lightly damped SOR pGMRES. Not restarted.	
...
namespace BlockSp
{
	...
	/// these are the user's responsibility to provide. \n
	...
	///For T real, floats and doubles only. 
	///See Y. Saad "Iterative Methods for Sparse Linear Systems" (chapter 9.2) for details. \n
	template<typename T, int B, int D, typename PreCond, typename SpMat = BlockSp_Compr<T, B* D>>
	solArray<T, B, D>	pCG(const SpMat& A, const solArray<T, B, D>& b,
	...
	void one_arnoldi(int iter, blitz::Array<T, 2>& H, std::vector<solArray<T, B, D>>& Qn,
		const SpMat& A, const PreCond* preCond, ScalarType_t<T> zero_thresh)
	{
		//resize matrix H
		int m = iter + 2;
		int n = iter + 1;
	...
	template<typename T, int B, int D, typename PreCond, typename SpMat = BlockSp_Compr<T, B* D>>
	solArray<T, B, D> pGMRES(const SpMat& A, const solArray<T, B, D>& b,
		ScalarType_t<T> tol, const PreCond& preCond, int maxIters = 1000, int restart = -1,
	...
	template<typename T, int M, typename PreCond, typename SpMat = BlockSp_Compr<T, M>>
	dense::Eigenvalues_Arr<T> arnoldi_eigenvalues(const SpMat& A, int iters = 30,
	...
	struct SOR_pGMRES
	{
		...
		///pGMRES with block SOR preconditioner/LU block inversion.
		///Defaults - SOR damping parameter of 1.6, 3 iterations per direction (6 total), symmetric SOR.
		///Note: The SOR damping parameter w may greatly impact solver speed/robustness.
		///w = (0, 2), larger parameters are often faster but smaller are more robust.
		///Note: Not restarted by default. 
		SOR_pGMRES(const BlockSp<T, B* D>& A, ScalarType_t<T> w = 1.6, int nu = 3, int SORtype = 0,
			ScalarType_t<T> tol = 1.0e-8, int maxIters = 5000, int restart = -1, int k = -1,
		...
		solArray<T, B, D> solve(const SpMat& A, const solArray<T, B, D>& b)
		{
			if constexpr (std::is_same_v<SpMat, BlockSp_Compr<T, B * D>>) //if A compressed, set pointer.
				access_SOR_pre().set_compr_ptr(A);
	...
	struct SOR_pCG
	{
		...
		///pCG with symmetric SOR preconditioner / Cholesky block inversion.
	...
	struct ILU0_pGMRES
	{
		...
		///Solve Ax = b, for x
		template<typename SpMat>
		solArray<T, B, D> solve(const SpMat& A, const solArray<T, B, D>& b) const
		{
	...
	///Note: these solvers use the base BlockSp sparse matrix, 
	/// not the fast compressed form. The preconditoner is also constructed each call.
	...
	namespace ez
	{
		...
		template<typename T, int B, int D = 1, typename SpMat = BlockSp_Compr<T, B* D>>
		solArray<T, B, D> sor_pGMRES(const SpMat& A, const solArray<T, B, D>& b,
			bool printIters = false, bool printEigInfo = false)
		{
