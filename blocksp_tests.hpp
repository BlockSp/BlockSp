...
///
///The first provided test solves a random symmetric BlockSp matrix with 
...
///		Matrices are converted from .mtx format to BlockSp using the mtxMatrix_to_BlockSp function.
///
///dense_test tests all dense matrix solvers.
...
namespace BlockSp::tests
{
		...
		bool noPre = false, bool diag = false, bool robust = false)
	{
		...
		//Compress A
		auto A_compr{ A.compress() }; //this creates a compressed BlockSp matrix,
		//which results in faster solves.
		...
		if (noPre)
		{
			//pGMRES, no preconditioner, no restart
			{
				...
				auto x{ pGMRES<T, B, D>(A_compr, b, tol, [](const auto& x) {}, maxIters, -1, -1, true) };
			...
			{
				start = std::chrono::steady_clock::now();
				auto x{ pGMRES<T, B, D>(A_compr, b, tol, [](const auto& x) {}, maxIters, restart, re_k, true) };
		...
		if (diag)
		{
			//no restart
			{
				...
				auto x{ pGMRES<T, B, D>(A_compr, b, tol, diag, maxIters, -1, -1, true) };
			...
			//with restart
			{
				start = std::chrono::steady_clock::now();
				preConditioners::Diag<dense::LUFactor<T, B* D>, T, B, D> diag(A);
				auto x{ pGMRES<T, B, D>(A_compr, b, tol, diag, maxIters, restart, re_k, true) };
		...
		//ILU0 pGMRES, not restarted
		{
			...
			auto x{ ilu0_pgmres.solve(A_compr, b) };
		...
		//SOR pGMRES with LU inversion, no restart
		{
			start = std::chrono::steady_clock::now();
			SOR_pGMRES<T, B, D> sor_pgmres(A);
			sor_pgmres.change_tol(tol); sor_pgmres.change_printIters(true);
			auto x{ sor_pgmres.solve(A_compr, b) };
			std::cout << "BlockSp SOR pGMRES time          " << time_since(start) << std::endl;
			if (have_sol) std::cout << "error                            " << infNorm(x - x_sol) << std::endl;
			std::cout << std::endl;
		}

		//SOR pGMRES, with LU inversion and deflated_restart
		{
			start = std::chrono::steady_clock::now();
			SOR_pGMRES<T, B, D> sor_pgmres_dr(A, 1.6, 3, 0, tol, 5000, 30, 10, true);
			auto x{ sor_pgmres_dr.solve(A_compr, b) };
		...
		if constexpr (!is_complex_v<T>)
			if (SPD)
			{
				...
				{
					start = std::chrono::steady_clock::now();
					auto x{ pCG<T, B, D>(A_compr, b, tol, [](const auto&) {}, maxIters, true) };
				...
				if (diag)
				{
					...
					auto x{ pCG<T, B, D>(A_compr, b, tol, diag, maxIters, true) };
				...
				{
					start = std::chrono::steady_clock::now();
					IChol0_pCG<T, B, D> ichol0_pcg(A, tol, 5000, true);
	...
	void LDG_2D_vecLap_test()
	{
		...
		auto A{ util::mtxMatrix_to_BlockSp<double, B * D>(mtx_A, 1, true) };
		...
		auto x{ util::mtxVector_to_solArray<double, B, D>(mtx_x, 1) };

		std::string mtx_b = "test_mats/LDG_2D_vecLap_b.mtx";
		auto b{ util::mtxVector_to_solArray<double, B, D>(mtx_b, 1) };
	...
	void rdb_test()
	{
		...
		auto A{ util::mtxMatrix_to_BlockSp<double, B>(rdb_mtxMat, 14) }; //matrix size starts on line 14 of .mtx file
		A.shrink_to_fit();
	...
	void qc_test(bool testNoPre = false)
	{
		...
		auto A{ util::mtxMatrix_to_BlockSp<std::complex<double>, B>(qc_mtxMat, 14, true) }; //matrix size starts on line 14 of .mtx file
		A.shrink_to_fit();
